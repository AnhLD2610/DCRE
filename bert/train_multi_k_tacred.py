...
class Manager(object):
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                for k in instance.keys():
                    ...
                    batch_instance['ids'] = torch.tensor([seen_des[self.id2rel[new_labels[m].item()]]['ids'][m%self.config.num_gen_augment] for m in range (len(new_labels))]).to(self.config.device)
                    batch_instance['mask'] = torch.tensor([seen_des[self.id2rel[new_labels[m].item()]]['mask'][m%self.config.num_gen_augment] for m in range (len(new_labels))]).to(self.config.device)
                ...
                repeated_hidden = torch.repeat_interleave(hidden, repeats=self.config.num_gen_augment, dim=0)
                ...
                for idx in range(len(new_labels)):
                    ...
                    loss3.append(online_contrastive_loss(rep_des, repeated_hidden, label_for_loss))
                loss3 = torch.stack(loss3).mean()

                loss_retrieval = MutualInformationLoss()

                loss2 = loss_retrieval(repeated_hidden, labels_des, new_matrix_labels_tensor)
                ...
                loss = 1*loss + 2*loss2 + 0.5*loss3 + 1*loss4 
    ...
    def eval_encoder_proto_des(self, encoder, seen_proto, seen_relid, test_data, rep_des):
        ...
        for batch_num, (instance, label, _) in enumerate(test_loader):
            ...
            rrf_logits = 4 / (rrf_k + logits_ranks)
            rrf_logits_des = 6 / (rrf_k + logits_des_ranks)
